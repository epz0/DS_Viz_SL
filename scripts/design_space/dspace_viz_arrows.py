...
def plot_full_ds(embed, sol_c, markers_ls, cvh_plot=True):
    ...
    for i in range(len(embed[:,0])):
        scatter = ax.scatter(embed[:, 0][i],embed[:, 1][i], c=sol_c[i], marker=markers_ls[i], s=15)
    ...
    if cvh_plot == True:
        ...
        ax.plot(embed[hullDS.vertices, 0], embed[hullDS.vertices, 1], 'o', mec='k', color='none', lw=1, markersize=18) #transparent markers, with solid edge
...
def plot_ds_exploration_arrows(dir_data, df_unm, embed, df_dist_metrics, sol_c, pt_unique, df_colors, save_plot=True, fs_cvh=True, lbl=False):
    ...
    for i in range(len(pt_unique)):
        ...
        rowsPT = df_unm[df_unm['OriginalID_PT'] == pt_unique[i]]
        ...
        prepost = df_unm['OriginalID_PrePost'][startPT:endPT+1]     # list of pre/post types
        labelPT = df_unm['OriginalID_Sol'][startPT:endPT+1]         # getting the labels for the current PT
        ...
        # plot annotations
        plt.axis('on')
        plt.title(f'{pt_unique[i]} DS Exploration [DS: 459 + 100 solutions]', fontsize=16, fontweight='bold')
        #plt.show()
...
def plot_ds_exploration_incremental_arrows(dir_data, df_unm, embed, df_dist_metrics, sol_c, pt_unique, df_colors, markers_list, save_plot=True, fs_cvh=True, lbl=False):
    ...
    for i in range(len(pt_unique)):
        ...
        prepost = list(df_unm['OriginalID_PrePost'][startPT:totalsol+startPT])     # list of pre/post types
        labelPT = list(df_unm['OriginalID_Sol'][startPT:totalsol+startPT])         # getting the labels for the current PT
        labelPT = [int(a) for a in labelPT]                          # float to int
        ...
        sol_c = df_unm['HEX-Win']

        for n in range(len(rowsPT)):
            plt.close()
            #* call function to plot DS
            f, ax = plot_full_ds(embed, sol_c, markers_list, cvh_plot=fs_cvh)                   # plot first part of the fig
            ...
            match n:
                ...
                case _:
                    ...
                    if n >= first_sol_pst:
                        #* first pst style (P)
                        ax.plot(x_y_points[first_sol_pst, 0], x_y_points[first_sol_pst, 1],  mec=color_edge, color='k', lw=2, markersize=20) #'P',

                    #print(f'totalsol - {totalsol}')
                    if n+1 == totalsol:
                        #print(n+1)
                        #* last style (s)
                        ax.plot(x_y_points[-1, 0], x_y_points[-1, 1],  mec=color_edge, color='k', lw=2, markersize=20) #'X',
