    ...
    # get metrics from convex hull created
        ...
        if save_plot == True:
            ...
            if mult_plot is not None:
                # plot annotations
                plt.title(f'DS-Viz ({pt_unique[i]} | 459 Part | 100 gallery), Full Session, all results, {mult_plot}', fontsize=16, fontweight='bold')
            else:
                plt.title(f'DS-Viz ({pt_unique[i]} | 459 Part | 100 gallery), Full Session, all results', fontsize=16, fontweight='bold')
            #plt.show()
...
def plot_pre(dir_data, df, pt_cvxh, embed, df_colors, df_DS_vertx, save_plot=False, mult_plot=None):
    ...
    pre_tag = 'KYY'                                                                                     #* string that tags pre intervention solutions (KYY in this case from the masking)
    ...
    # get metrics from convex hull created
        ...
        if save_plot == True:
            ...
            if mult_plot is not None:
                # plot annotations
                plt.title(f'DS-Viz ({pt_unique[i]} | 203 Part | 100 gallery), Pre, all results, {mult_plot}', fontsize=16, fontweight='bold')
            else:
                plt.title(f'DS-Viz ({pt_unique[i]} | 203 Part | 100 gallery), Pre, all results', fontsize=16, fontweight='bold')
            #plt.show()
...
def plot_post(dir_data, df, pt_cvxh, embed, df_colors, df_DS_vertx, save_plot=False, mult_plot=None):
    ...
    post_tag = 'ZSB'                                                                                    #* string that tags pre intervention solutions (KYY in this case from the masking)
    ctx_tag = 'UNK'                                                                                     #* string that tag context solutions (UNK from the gallery solutions)
...
def plot_embedding(dir_data, df, embed=None, embed_name=None, Wg='W2', NN=115, MD=0.05, densm=2):
    ...
    if my_graph.is_file():
        ...
        embedding_umap = pd.read_csv(Path(f'{dir_exp}/DS_{embed_name}.csv'))
        embed = embedding_umap[['x','y']].to_numpy()
