...
def dist_metrics_fs(df_unmask, dist_matrix):
    ...
    pt_unique = df_unmask['OriginalID_PT'].unique()
    pt_unique = pt_unique[:-1]                      #drop the gallery

    # output df_dist
    df_dist = df_unmask[['OriginalID_PT', 'OriginalID_Group']].drop_duplicates().reset_index(drop=True)
    df_dist = df_dist.drop([30])
    ...
    for i in range(len(pt_unique)):
        ...
        rows_PT = df_unmask[df_unmask['OriginalID_PT'] == pt_unique[i]] # get the lines corresponding to each participant
        ...
        for j in solutions_list:
            solution_idx.append(rows_PT.index[rows_PT['OriginalID_Sol'] == j].tolist()[0])
...
def dist_metrics_pre(df_unmask, df_dist, pt_unique, dist_matrix):
    ...
    df_PRE = df_unmask[df_unmask['OriginalID_PrePost'].eq("Pre")]

    for i in range(len(pt_unique)):
        rows_PT = df_PRE[df_PRE['OriginalID_PT'] == pt_unique[i]] # get the lines corresponding to each participant
...
def dist_metrics_post(df_unmask, df_dist, pt_unique, dist_matrix):
    ...
    df_PST = df_unmask[df_unmask['OriginalID_PrePost'].eq("Pst")]

    for i in range(len(pt_unique)):
        # get list with all solutions for the participant
        nsol_PT_all = len(df_unmask[df_unmask['OriginalID_PT'] == pt_unique[i]])+1
        solutions_list_all = list(range(nsol_PT_all))  # getting the list
        solutions_list_all.pop(0)

        # get list with pre solutions
        nsol_PT_pre = len(df_unmask[(df_unmask['OriginalID_PT'] == pt_unique[i]) & (df_unmask['OriginalID_PrePost'] == 'Pre')])+1
        solutions_list_pre = list(range(nsol_PT_pre))  # getting the list
        solutions_list_pre.pop(0)

        rows_PT = df_PST[df_PST['OriginalID_PT'] == pt_unique[i]] # get the lines corresponding to each participant
