...
def novelty_from_neig(dir_data, df, embed, delta, save_df=False, save_plot=True, fn=None):
    ...
    Returns:
        df_dens: List with novelty (NN) values for each solution.
    """
    ...
    if save_plot == True:
        ...
        df_topten = df_dens.loc[df_dens['novel_nn'] >= topperc].copy()
        df_botten = df_dens.loc[df_dens['novel_nn'] <= botperc].copy()

        sns.set(style="ticks", context="talk")
        plt.style.use("default")

        fig, axes = plt.subplots(layout='constrained', figsize=(10, 10))
        plot = sns.kdeplot(data=df_dens, x="x_emb", y="y_emb", fill=True, levels=500, cmap='mako')
        ...
        plt.savefig(f'{dir_exviz}/EX_Novelty_NN_{fn}.tiff', dpi=300, bbox_inches='tight')
        print('KDE plot - done!')
...
def novelty_nn_study(dir_data, embed, delta_r=[0.05,1.05], type_hist='probability', x_fixed=True, bin=10):
    ...
    plt.savefig(f'{dir_exviz}/EX_Novelty-NN-delta-{type_hist}.png', dpi=300, bbox_inches='tight')
