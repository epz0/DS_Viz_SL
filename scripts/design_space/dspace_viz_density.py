...
def prep_density(df, embed):
    ...
    df_kde = df[['FullID','ParticipantID', 'GroupID', 'PrePost', 'result', 'type']].copy()
...
def plot_single_density(dir_data, df_dens, x_lim, y_lim, lvls=None, cmapn=None, figtitle=None, fn=None):
    ...
    plt.savefig(f'{dir_exviz}/{fn}.tiff', dpi=300, bbox_inches='tight')
    plt.close()
...
def multiplot_kde_cvh(dir_data, df_dens, df_color, x_lim, y_lim, nrows, ncols, axbase, param=None, lvls=None, cmapn=None, kde=True, cvxh=True, precvh=False, postcvh=False, allscatter=False, figtitle=None, fn=None):
    ...
    for (n, axi) in zip(range(len(ids)), axes.flat):
        ...
        if param == 'ParticipantID':
            pt_color = pt_color = df_color.loc[n,'HEX-Win']
        else:
            pt_color = 'red'
        ...
        if cvxh == True:
            ...
            if precvh == True:
                dfpre = dfsubset[dfsubset['PrePost'] == 'KYY'].copy()
                if len(dfpre) > 3:
                    x_pre = dfpre['x_emb']
                    y_pre = dfpre['y_emb']
            ...
            if postcvh == True:
                dfpost = dfsubset[dfsubset['PrePost'] == 'ZSB'].copy()
                if len(dfpost) > 3:
                    x_post = dfpost['x_emb']
                    y_post = dfpost['y_emb']
...
def novelty_from_density(dir_data, df_dens, x_lim, y_lim, res=1000j, prt_integral=True, plot=False, save_metrics=True):
    ...
    if prt_integral == True:
        ...
        integinf = integrate.nquad(f, [[0, 20], [0, 20]])[0]
        print(f'Double integral of KDE function over area delimited by x[0,20], y[0,20]: {integinf:.3f}')
    ...
    if plot == True:
        ...
        plt.savefig(f'{dir_exviz}/Density_KDE_Novelty.svg', format='svg', dpi=300, bbox_inches='tight')
        plt.close()
        ...
        axes.set(xlabel=None, ylabel=None)
        plt.savefig(f'{dir_exviz}/EX_Novelty_all_mako.tiff', dpi=300, bbox_inches='tight')
